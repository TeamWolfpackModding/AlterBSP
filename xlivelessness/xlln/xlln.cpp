...
static DWORD WINAPI ThreadProc(LPVOID lpParam)
{
	...
	wchar_t title[80];
	swprintf_s(title, 80, L"AlterBSP Login");
...
static LRESULT CALLBACK DLLWindowProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	if (message == WM_PAINT) {
		...
		if (wParam == MYMENU_EXIT) {
			...
			MessageBox(hwnd,
"An adaptation of GlitchyScripts's Halo ''XLiveLessNess'' plugin,\n\
rewritten by Team Wolfpack to work with BSP.\n\
\n\
\n\
\n\
\n\
GPL v2 Copyright (C) 2020"
				, "About", MB_OK);
		}
		...
		CreateWindowA("button", "Login", WS_CHILD | WS_VISIBLE | WS_TABSTOP,
			10, 40, 150, 22, hwnd, (HMENU)MYWINDOW_BTN_LOGIN, xlln_hModule, NULL);

		CreateWindowA("button", "Logout", WS_CHILD | WS_TABSTOP,
			10, 40, 150, 22, hwnd, (HMENU)MYWINDOW_BTN_LOGOUT, xlln_hModule, NULL);
